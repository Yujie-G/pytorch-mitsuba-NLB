...
class Merl:
    ...
    def to_dataframe(self, mode='interpolation', angles_only=False):
        ...
        theta_h = np.linspace(0, np.pi / 2, self.sampling_theta_h)
        theta_d = np.linspace(0, np.pi / 2, self.sampling_theta_d)
        phi_d = np.linspace(0, np.pi, self.sampling_phi_d)
        TH, TD, PD = np.meshgrid(theta_h, theta_d, phi_d, indexing='ij')
        cols = np.vstack([TH.ravel(), TD.ravel(), PD.ravel()]).T
        # cols = np.array(cols)
